...
return { conversation: "hello, i'm UpaBot-MD" }},   
msgRetryCounterMap,
logger: pino({ level: 'silent' }),
auth: state,
browser: ['UpaBot-MD','Edge','1.0.0'],
version   
}       
...
logger: pino({ level: 'silent' }),
auth: state,
browser: ['UpaBot-MD','Edge','107.0.1418.26'],
version   
}*/
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "UpaJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '4', '-type', 'f', '-delete'])))
}, 30 * 1000)}
...
let prekey = []
let directorio = readdirSync("./UpaBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') || file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./UpaBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
let listaDirectorios = readdirSync('./UpaJadiBot/');
//console.log(listaDirectorios) Nombra las carpetas o archivos
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./UpaJadiBot/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./UpaJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') || fileInDir.startsWith('app-') || fileInDir.startsWith('session-')
})
SBprekey = [...SBprekey, ...DSBPreKeys]
DSBPreKeys.forEach(fileInDir => {
unlinkSync(`./UpaJadiBot/${directorio}/${fileInDir}`)
})
}
})
...
const directories = ['./UpaBotSession/', './UpaJadiBot/']
